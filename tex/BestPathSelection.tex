...
Usualy set: & automatically when injecting prefixes into BGP by originating AS\\
Mandatory attribute \\
\end{tabular}
...
Be aware that the value of this attribute can be overwritten by any BGP speaking router. At RIPE91 James Bensley presented that some transit providers change Origin Type on purpose to attract more traffic\cite{ripe91jborigin}.

Therefore if you are a stub-AS (having no other ASes as customers) you might consider rewriting Origin Type on all received prefixes to \emph{IGP}.
